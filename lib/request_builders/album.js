...
var removeAlbum = exports.removeAlbum = function removeAlbum(_ref5, _ref6, cb) {
...
var removeImage = exports.removeImage = function removeImage(_ref7, _ref8, cb) {
  ...
  _async2.default.waterfall([function (done) {
    redisClient.zrange('order_id_' + order_id + ':files', [0, -1], function (err, files) {
      done(err, files);
    });
  }, function (files, done) {
    var fileObj = files.find(function (file) {
      return file.filename === filename && file.album_id === album_id;
    });

    if (fileObj) {
      redisClient.zrem('order_id_' + order_id + ':fiiles', fileObj, function (err, data) {
        if (err) return done(err);
        done(null, { count: 1 });
      });
    } else {
