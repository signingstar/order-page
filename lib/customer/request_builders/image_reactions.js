...
var parseReactions = function parseReactions(obj, userId, albumId) {
  ...
  for (var index in obj) {
    ...
    if (index === userId) {
      reactionObj[_actions.LIKES] = +reaction;
      reactionObj[_actions.LIKED].push({ name: 'You', reaction_type: reaction });
    } else {
      reactionObj[_actions.LIKED].push({ name: user_name, reaction_type: reaction });
    }
  }
