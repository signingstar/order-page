  ...
  return {
    ...
    getReaction: function getReaction(_ref5) {
      ...
      _async2.default.waterfall([function (done) {
        ...
        return getImageReactions({ order_id: order_id, undefined: undefined, user_id: user.id, files: files }, function (err, res) {
    ...
    fetchImagesByUser: function fetchImagesByUser(_ref9) {
      ...
      var user_name = body.user_name,
          order_id = body.order_id;
      var fetchImagesByUser = RequestBuilder.fetchImagesByUser;


      fetchImagesByUser({ order_id: order_id, user: user, reaction: reaction, image_id: image_id }, function (err, result) {
        if (err) return res.status(500).end();
        res.status(200).end();
      });
    },

    finalizeOrderByCustomer: function finalizeOrderByCustomer(_ref10) {
      ...
      (0, _api_executor.finalizeCustomerOrder)({ params: params, body: body, userId: user.id }, { logger: logger, queryDb: queryDb }, function (_ref11) {
        var err = _ref11.err,
            orderData = _ref11.orderData,
