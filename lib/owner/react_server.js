...
var ReactComponent = function ReactComponent(_ref, _ref2, cb) {
  ...
  var _requestBuilder = (0, _request_builders2.default)({ redisClient: redisClient, queryDb: queryDb, logger: logger }),
      products = _requestBuilder.products,
      categories = _requestBuilder.categories,
      viewOrder = _requestBuilder.viewOrder;

  var requests = { products: products, categories: categories };

  _async2.default.waterfall([function (done) {
    if (pageInProgress) {
      (0, _view_order.viewInProgressOrder)([userid], { logger: logger, queryDb: queryDb }, function (err, res) {
        if (res && res.length) {
    ...
    if (pageInProgress) {
      if (orderid) {
        var orderInfo = viewOrder({ orderid: orderid, userid: userid });
