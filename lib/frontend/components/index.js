...
var restoreFromSessionStorage = function restoreFromSessionStorage() {
  ...
  if ((!preloadedState || Object.keys(preloadedState.selectionState).length < 3) && sessionStoreStr) {
    ...
    var category = preloadedState.categoryState.category;

    var sessionCategory = sessionStore.categoryState.category;
    var shouldRestoreSessionStore = category ? category === sessionCategory : false;
